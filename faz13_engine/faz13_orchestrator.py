# ================================================================
# FAZ-13 ORCHESTRATOR — TEAM-FIRST / LEAGUE AVG DISABLED
...
# Sadece periyot dağılımı ve dar bant toleransı (lig ortalaması DEĞİL)
LEAGUE_PROFILE = {
    "NBA": {"band_half": 6.0, "weights": [0.24, 0.25, 0.25, 0.26]},  # ±5–7
...
def run_faz13_auto_pipeline(
    ...
    """
    TEAM-FIRST:
      base_pred = home_avg_for + away_avg_for
    LIG ORTALAMASI: KAPALI
    """
    ...
    # Fail-safe prior (UNKNOWN yok) — lig ortalaması değil, güvenli başlangıç
    if home_avg <= 0: home_avg = 111.0 if lg == "NBA" else 80.0
    if away_avg <= 0: away_avg = 111.0 if lg == "NBA" else 80.0
    ...
    out = {
        "match": {"league": lg, "date": date_str, "home": home, "away": away},
        "baseline": {"home_avg_for": round(home_avg, 2), "away_avg_for": round(away_avg, 2), "inj_count": inj_count},
        "base_pred": round(base_pred, 1),
        "band": band,
        "periods": periods,
        "market": {"line": market_line, "delta": market_delta, "fallback_used": fallback_used, "provider": market.get("provider")},
        "ou": {"dir": ou_dir, "band": band},
        "confidence": round(conf, 3),
        "risk": risk,
        "debug": {"context_keys": list(ctx.keys())},
    }
