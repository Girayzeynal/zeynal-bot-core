...
from faz22_engine import faz22_apply_hint
...
def faz23_apply_result(
    ...
    rec = memory_get(key)
    if not rec:
        return {"engine": "FAZ-23-FEEDBACK", "ts": ts, "key": key, "error": "no_memory_record",
                "tags": ["NO_RECORD"], "abs_error": None, "hit_band": None, "meta_delta_hint": None, "new_weights": None}
    ...
    if meta_delta_hint in ("+market_weight", "-market_weight"):
        try:
            ...
            tags.append("FAZ22_WEIGHT_UPDATE_FAIL")
    ...
    return {"engine": "FAZ-23-FEEDBACK", "ts": ts, "key": key, "error": None,
            "tags": tags, "abs_error": abs_err, "hit_band": hit_band,
            "meta_delta_hint": meta_delta_hint, "new_weights": new_weights}
