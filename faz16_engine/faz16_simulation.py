...
def faz16_live_recalibrate(
    ...
    *,
    ...
    early_window_min_sec: int = 6 * 60,   # 6 minutes into game (reserved for future gating)
    ...
    # (kept for future logic; do not remove)
    ...
    if sec_elapsed_total <= 60:  # first minute is chaos
        return Faz16LiveResult(
            live_mean_total=float(prematch_sim_mean),
            live_std_total=float(prematch_sim_std),
            pace_delta_pct=0.0,
            mean_shift=0.0,
    ...
    if market_total is None:
        return Faz16LiveResult(
            live_mean_total=round(float(live_mean), 2),
            live_std_total=round(float(live_std), 2),
            pace_delta_pct=round(float(pace_delta_pct), 2),
...
def faz16_band(mean: float, std: float, sigma: float = 0.55) -> Dict[str, float]:
    ...
    lo = float(mean) - half
    hi = float(mean) + half
    return {"lo": round(lo, 1), "hi": round(hi, 1), "center": round(float(mean), 1)}
...
def faz16_run_simulation(*args, **kwargs):
    ...
    try:
        ...
        return {"error": "FAZ16_RUN_SIMULATION_FAILED", "detail": str(e)}from __future__ import annotations
...
def faz16_run_simulation(*args, **kwargs):
    ...
    try:
        ...
        return {"error": "FAZ16_RUN_SIMULATION_FAILED", "detail": str(e)}
