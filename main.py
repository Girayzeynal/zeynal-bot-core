...
def simulate_nba_cmd(message):
    ...
    for r in simulation_results:
        ...
        reply += f"☑️ Kazanan: {r['pick']} ({int(r['confidence']*100)}%)\n\n"
...
def faz6_test_cmd(message):
    bot.reply_to(message, run_faz6_engine(mode="test"), parse_mode="Markdown")

@bot.message_handler(commands=["faz6_auto"])
def faz6_auto_cmd(message):
    bot.reply_to(message, run_faz6_engine(mode="auto"), parse_mode="Markdown")

@bot.message_handler(commands=["faz6_risk"])
def faz6_risk_cmd(message):
    bot.reply_to(message, run_faz6_engine(mode="risk"), parse_mode="Markdown")

@bot.message_handler(commands=["faz6_edge"])
def faz6_edge_cmd(message):
    bot.reply_to(message, run_faz6_engine(mode="edge"), parse_mode="Markdown")

@bot.message_handler(commands=["faz6_real"])
def faz6_real_cmd(message):
    bot.reply_to(message, run_faz6_engine(mode="real"), parse_mode="Markdown")

@bot.message_handler(commands=["faz6_balance"])
def faz6_balance_cmd(message):
    bot.reply_to(message, run_faz6_engine(mode="balance"), parse_mode="Markdown")
...
if __name__ == "__main__":
    main()from faz6_engine.faz6_engine_main import run_faz6_engine
from faz5_engine.heavy_engine_main import run_heavy_engine
