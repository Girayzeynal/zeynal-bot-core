...
def register_daily_stats(conf: float, edge: float):
    mem = load_memory()
    mem["days"].append({"ts": int(time.time()), "conf": float(conf), "edge": float(edge)})
    if len(mem["days"]) > 7:
        mem["days"] = mem["days"][-7:]
    save_memory(mem)
    log.info(f"[FAZ-7.9] Günlük kayıt: conf={conf:.3f}, edge={edge:.3f}")
...
def _faz92_behavior_curves(df: pd.DataFrame, slope: float, vol: float) -> dict:
    ...
    last = float(conf.iloc[-1])
    first = float(conf.iloc[0])
    change = last - first

    std_conf = float(conf.std() if len(conf) > 1 else 0.0)

    stability = 1.0 / (1.0 + std_conf * 25.0)
    stability = max(0.0, min(stability, 1.0))

    momentum = max(-0.1, min(change, 0.1))

    denom = std_conf + 0.02
    tci = max(0.0, min(abs(slope) / denom, 1.0))

    noise_ratio = max(0.0, min(vol / (abs(change) + 0.01), 1.5))
...
def faz79_brain():
    ...
    try:
        ...
        log.warning(f"[FAZ-7.9] slope hesap hatası (SVD fallback): {e}")
        slope = 0.0

    ema_c = _ema(df["conf"])
    if slope > 0.01 and ema_c >= avg_conf:
        trend = "UP"
    elif slope < -0.01 and ema_c <= avg_conf:
        trend = "DOWN"
    else:
        trend = "FLAT"
...
# ================================================================
# 🔥 GLOBAL LIVE (NBA / EL / TR / EU / ID MODE)  → /live
# ================================================================
@bot.message_handler(commands=["live"])
def cmd_live_global(message):
    """
    Hibrit kullanım (GLOBAL API):
    ...
    try:
        ...
        log.error("[LIVE CMD] Genel hata: %s", e, exc_info=True)
        bot.reply_to(
            message,
...
# ================================================================
# 🌍 FAZ-13 LIVE HYBRID KOMUT (/live13 – ID + takım adı hibrit)
# ================================================================
def _parse_live_command_args(text: str) -> dict:
    """
    /live13 komutu için hibrit parser:
      - /live13           → boş
      - /live13 4412200   → match_id
      - /live13 NBA LAL BOS → league + home + away
      - /live13 LAL BOS   → home + away
    Dönen dict:
      {
        "league": str | None,
        "home": str | None,
    ...
    # Sadece ID → /live13 4406870
    if len(args) == 1 and args[0].isdigit():
        info["match_id"] = args[0]
        return info

    # 3 parça → /live13 NBA LAL BOS (lig + takımlar)
    if len(args) == 3:
    ...
    try:
        ...
        if not info["raw_args"]:
            bot.reply_to(
                message,
                "📡 Kullanım:\n"
                "<code>/live13 4412200</code>  (Maç ID)\n"
                "<code>/live13 NBA LAL BOS</code> (Lig + Takımlar)\n"
                "<code>/live13 LAL BOS</code>  (Takımlar)\n",
            )
            return
...
def _vision_worker(img_bytes: bytes) -> dict:
    ...
    # Gerçek OpenAI Vision entegrasyonu burada olacak.
...
def ultra_ocr_engine_v3(img_bytes: bytes) -> dict:
    ...
    if Image and pytesseract:
        futures[OCR_EXECUTOR.submit(_tesseract_worker, img_bytes)] = (
            "TESSERACT",
            TESSERACT_TIMEOUT,
        )

    if easyocr is not None:
        futures[OCR_EXECUTOR.submit(_easyocr_worker, img_bytes)] = (
            "EASYOCR",
            EASYOCR_TIMEOUT,
        )

    if VISION_MODE and openai:
        futures[OCR_EXECUTOR.submit(_vision_worker, img_bytes)] = (
            "VISION",
            VISION_TIMEOUT,
        )
...
def cmd_visual_match(message):
    ...
    bot.reply_to(
        ...
        "Ultra OCR Engine v3 (A+B+C) tarayıp FAZ-13 + GOD-LAYER pipeline'a sokacağım.",
    )
...
def cmd_visual_upload_extreme(message):
    ...
    Ultra OCR Engine v3:
      ...
      - FAZ-13 C MODE FULL POWER + GOD-LAYER
    """
    try:
...
def setup_webhook():
    ...
    if WEBHOOK_URL:
        for attempt in range(1, 3):
            try:
                log.info(f"[FAZ-CORE] Webhook deneme {attempt}: {WEBHOOK_URL}")
                bot.set_webhook(url=WEBHOOK_URL)
                log.info("[FAZ-CORE] Webhook başarıyla set edildi.")
                break
            except Exception as e:
                log.error(f"[FAZ-CORE] Webhook set hatası (deneme {attempt}): {e}")
                time.sleep(1.5)
    else:
